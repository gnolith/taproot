...
export const TAPROOT_SCHEMA_VERSION = '5';
...
export async function verifyTaprootPackageSeeds(
  ...
  const migrationSeedTotal = await db
    ...
    .all<{ count: number }>();
  if (
    Number(migrationSeeds.results[0]?.count ?? 0) !== 6 ||
    Number(migrationSeedTotal.results[0]?.count ?? 0) !== 6
  )
    throw new SchemaMismatchError(
...
export async function inspectTaprootSchema(
  ...
  const required = catalogObjectNames(taprootSchemaStatements, 'TABLE');
  ...
  const requiredIndexes = catalogObjectNames(taprootSchemaStatements, 'INDEX');
